...
The microphones form one stereo I2S input: tie both BCLK lines together, both WS lines together, and both DATA lines together at GPIO13. Set the reference INMP441 L/R pin to LEFT and the error INMP441 L/R pin to RIGHT. Do not connect the two microphone DATA outputs to separate GPIOs; one I2S controller has one data input in this configuration. This no-DAC build is monitor-only: it sends measurements to the laptop over USB and cannot play anti-noise into headphones.
