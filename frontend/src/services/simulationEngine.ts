...
export type NoiseProfile = 'ENGINE' | 'GENERATOR' | 'MACHINERY' | 'ROTOR' | 'MIXED' | 'QUIET'
...
const frequencies: Record<NoiseProfile, number> = { ENGINE: 118, GENERATOR: 92, MACHINERY: 740, ROTOR: 164, MIXED: 210, QUIET: 0 }
export function generateFrame(profile: NoiseProfile, mode: SimulationMode, sampleRate = 16000, size = 256, phase = 0): SimulationFrame {
  ...
  const targetReduction = mode === 'OFF' ? 0 : mode === 'FIXED' ? 0.55 : mode === 'SAFE' ? 0.15 : profile === 'QUIET' ? 0 : 0.72
  for (let index = 0; index < size; index += 1) {
    ...
    const machinery = profile === 'MACHINERY' || profile === 'MIXED' ? (Math.sin(index * 12.7) * 0.09) : 0
    const sample = base + machinery + Math.sin(index * 0.71 + phase) * 0.025
  ...
  return { input, output, residual, inputRms, outputRms, reduction: inputRms ? Math.max(0, (1 - outputRms / inputRms) * 100) : 0, dominantFrequency: frequencies[profile], confidence: profile === 'QUIET' ? 0.91 : 0.82, latencyMs: (size / sampleRate) * 1000 }
}
