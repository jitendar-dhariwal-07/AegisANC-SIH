...
void initAudio() {
  i2s_config_t input = {};
  input.mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_RX); input.sample_rate = SAMPLE_RATE; input.bits_per_sample = I2S_BITS_PER_SAMPLE_32BIT; input.channel_format = I2S_CHANNEL_FMT_ONLY_LEFT; input.communication_format = I2S_COMM_FORMAT_I2S; input.intr_alloc_flags = ESP_INTR_FLAG_LEVEL1; input.dma_buf_count = 4; input.dma_buf_len = 64; input.use_apll = false; input.tx_desc_auto_clear = false; input.fixed_mclk = 0;
  i2s_pin_config_t inputPins = { .bck_io_num = MIC_BCLK, .ws_io_num = MIC_WS, .data_out_num = I2S_PIN_NO_CHANGE, .data_in_num = REFERENCE_DATA };
  i2s_driver_install(I2S_NUM_0, &input, 0, nullptr); i2s_set_pin(I2S_NUM_0, &inputPins);
  i2s_config_t output = input; output.mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_TX);
...
void loop() {
  ...
  int32_t raw = 0; size_t received = 0;
  if (i2s_read(I2S_NUM_0, &raw, sizeof(raw), &received, 20) != ESP_OK || received != sizeof(raw)) { ++dmaUnderruns; return; }
  const int16_t reference = (int16_t)(raw >> 14); const bool safe = mode == AncMode::SAFE || abs(reference) > 30000; if (safe) mode = AncMode::SAFE;
  const bool adapt = mode == AncMode::FIXED || mode == AncMode::AI_ADAPTIVE; const int16_t output = mode == AncMode::OFF ? 0 : filter.process(reference, reference, adapt, mode == AncMode::AI_ADAPTIVE ? LEARNING_RATE : 0.001f);
  size_t written = 0; if (i2s_write(I2S_NUM_1, &output, sizeof(output), &written, 20) != ESP_OK || written != sizeof(output)) ++dmaOverruns;
